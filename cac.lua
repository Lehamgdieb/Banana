...
function bearlib:MakeWindow(cfg)
    ...
    function window_wrapper:MakeTab(tab_cfg)
        ...
        function tab_wrapper:AddSection(elem_cfg)
            local title = type(elem_cfg) == "string" and elem_cfg or elem_cfg[1] or elem_cfg.Name or elem_cfg.Title or elem_cfg.Section or "Section"
            local fluent_para = fluent_tab:AddParagraph({
                Title = title,
                Content = ""
            })
            local section_wrapper = {}
            function section_wrapper:SetTitle(newTitle)
                fluent_para:SetTitle(newTitle)
            end
